...
public class DWalletMerchantGatewayConfigurationView extends ConfigurationView {
	private POSTextField tfDWalletEmail;
	
	private JPasswordField tfDwalletPass;
	private DoubleTextField tfPointsConversion;
...
//	private String link = "http://reseller.authorize.net/application/?resellerId=27144"; //$NON-NLS-1$
	...
	public DWalletMerchantGatewayConfigurationView() {
...
//		add(lblMagneticCardReader, "cell 0 3,alignx leading"); //$NON-NLS-1$
		...
		JLabel lblDwalletEmail = new JLabel("DWallet Email"); //$NON-NLS-1$
		add(lblDwalletEmail, "cell 0 5,alignx leading"); //$NON-NLS-1$

		tfDWalletEmail = new POSTextField();
		add(tfDWalletEmail, "cell 1 5,growx"); //$NON-NLS-1$

		JLabel lblPassword = new JLabel("DWallet Password"); //$NON-NLS-1$
		add(lblPassword, "cell 0 6,alignx leading"); //$NON-NLS-1$


		tfDwalletPass = new JPasswordField();
		add(tfDwalletPass, "cell 1 6,growx"); //$NON-NLS-1$
		
		JLabel lblPointsConversion = new JLabel("Points per Dollar");
		add(lblPointsConversion, "cell 0 7,alignx leading");
		
		tfPointsConversion = new DoubleTextField(10);
		add(tfPointsConversion, "cell 1 7,growx");
...
//		CardReader card = CardConfig.getCardReader();
		...
		String dwalletEmail = CardConfig.getDWalletEmail();
		if (dwalletEmail != null) {
			tfDWalletEmail.setText(dwalletEmail);
		}

		String dWalletPass = CardConfig.getDWalletPassword();
		if (dWalletPass != null) {
			tfDwalletPass.setText(dWalletPass);
		}
		
		String dWalletPointConversion = CardConfig.getDWalletPointConversion();
		if (dWalletPointConversion != null) {
			tfPointsConversion.setText(dWalletPointConversion);
		}
...
//		cbSandboxMode.setSelected(CardConfig.isSandboxMode());
	...
	public void setMerchantDefaultValue(String accountNo, String pass) {
		tfDWalletEmail.setText(accountNo); //$NON-NLS-1$
		tfDwalletPass.setText(pass); //$NON-NLS-1$
	}
...
//		CardReader selectedItem = (CardReader) cbPaymentType.getSelectedItem();
...
//		}
	...
	private DefaultComboBoxModel<PaymentTypeWallet> createComboBoxModel(Vector items) {
...
//
//		CardReader currentReader = (CardReader) cbPaymentType.getSelectedItem();
...
//		}
//
//		cbPaymentType.setModel(createComboBoxModel(readers));
//		if (readers.contains(currentReader)) {
//			cbPaymentType.setSelectedItem(currentReader);
//		}
//
...
//
//		CardReader cardReader = (CardReader) cbPaymentType.getSelectedItem();

		CardConfig.setDWalletEmail(tfDWalletEmail.getText());
		CardConfig.setDWalletPassword(new String(tfDwalletPass.getPassword()));
		CardConfig.setDWalletPointConversion(tfPointsConversion.getText());

	...
	public void doSettle(PaymentType paymentType, double tenderAmount, CustomPayment customPayment) throws Exception {
		...
		switch (paymentType) {
			...
			case DWALLET:
				...
				if (dWalletDialog.isCanceled())
					return;
				
				transaction = paymentType.createTransaction();
				transaction.setCustomPaymentFieldName("DWALLETFIELD");
				transaction.setCustomPaymentName("DWALLET");
				transaction.setCustomPaymentRef("DWALLETREF");
				transaction.setTicket(ticket);
				transaction.setCaptured(true);
				setTransactionAmounts(transaction);
